...
ADMIN_EMAIL = "taqwaasif76@gmail.com"
...
def detect_country(text):
    # IMPORTANT: use word boundaries so words such as "us" inside normal
    # sentences (for example, "for us") are NOT mistaken for USA.
    import re
    t = text.lower()
    if re.search(r"\b(pakistan|pakistani)\b", t) or re.search(r"\b(pkr|rs)\b", t) or "rupees" in t:
        return "Pakistan"
    if (
        re.search(r"\b(usa|u\.s\.?|us|america|american|usd|dollars?)\b", t)
        or "$" in t
    ):
        return "USA"
    return None
...
def pricing_reply(country, service=None):
    # A specific country always wins over the user's saved country.
    if country not in {"Pakistan", "USA"}:
        return "💎 Please choose **Pakistan** or **USA** so I can show the correct pricing."
...
def pricing_conversion_note(source_country, target_country):
    # Approximate conversion for convenience only; the official service range
    # remains the authoritative quoted price.
    if source_country == target_country or not source_country or not target_country:
        return ""
    if source_country == "Pakistan" and target_country == "USA":
        return "\n\n💱 I can also give an approximate USD conversion of the Pakistan range for reference. The final quote remains scope-based."
    if source_country == "USA" and target_country == "Pakistan":
        return "\n\n💱 I can also give an approximate PKR conversion of the USA range for reference. The final quote remains scope-based."
    return ""
...
class AIAgent:
    ...
    def think(self, user_input):
        ...
        # Pricing/payment is handled deterministically so phrases such as
        # "pricing system", "payment method", or "how much" never fall through
        # to the general model and produce an unnecessary error.
        pricing_words = [
            "payment", "payments", "payment method", "payment methods", "pay",
            "pricing", "price", "cost", "costs", "charges", "budget",
            "fee", "fees", "rate", "rates", "how much", "pricing system",
            "price list", "pricing list"
        ]
        service = detect_service(user_input)
        country_in_message = detect_country(user_input)
        saved_country = self.user_info.get("country")
        country = country_in_message or (saved_country if saved_country in {"Pakistan", "USA"} else None)
        ...
        if any(word in lower for word in pricing_words) or self.pending_pricing_service is not None:
            ...
            if country_in_message:
                # Explicit country in the current message always overrides the
                # saved profile country for this particular pricing request.
                self.user_info["country"] = country_in_message
            ...
            if country:
                ...
                reply = pricing_reply(country, service)
                self.memory.append({"role": "assistant", "content": reply})
                # Generic pricing/payment questions show the complete five-service table.
                return {"reply": reply, "should_connect": False, "pricing_table": service is None}

            # Generic pricing/payment request with no known country: ask once, then
            # remember whether the user asked for one service or the complete table.
            self.pending_pricing_service = service if service else "__ALL__"
            reply = (
                "💎 I can give you the correct pricing. Which country are you pricing for — "
                "**Pakistan or USA**?"
            )
            self.memory.append({"role": "assistant", "content": reply})
            return {"reply": reply, "should_connect": False}
...
- If the user asks pricing/payment, use the saved user country when available; an explicitly mentioned country in the current message overrides it.
- For generic pricing/payment/payment-method questions with no service specified, provide the complete five-service pricing table.
- For a specific service and country, provide only that service's range.
- If the user wants a currency conversion, provide an approximate conversion for convenience and clearly label it as approximate; the listed Pakistan/USA service ranges remain the authoritative pricing.
"""
